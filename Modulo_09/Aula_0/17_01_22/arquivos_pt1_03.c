...
*/
	//ler com a funçao fgetc() o ponteiro arq e copia para o arquivo criado e setado pelo ponteiro carq, ate que ch = EOF.	
	while((ch = fgetc(arq)) != EOF) 
	{
		printf("%c",ch); // ch é inteiro pois a funçao fgetc() retorna int, mas não da problemas aparentes se for recebido por um char.
		fputc(ch, carq);
	}
